...
import { merge } from '../helper/common.js';
...
const prerenderStyle = (defaultMode = 'system', customCss) => {
    if (!loadedColorScheme) {
        ...
        ReactDOM.createRoot(document.createElement('div')).render(React.createElement(GlobalColorScheme, { "$defaultMode": _defaultMode, "$root": css `
                    ${loadedVars('root')}
                `, "$light": css `
                    ${loadedVars('light')}
                `, "$dark": css `
...
export { createTheme, detectMode, getMode, mergeStyleOverrides, prerenderStyle, reflectMode, restoreMode, setMode, themeChanged };
